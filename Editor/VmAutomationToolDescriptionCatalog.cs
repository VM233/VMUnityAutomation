        ...
        internal static string Get(string route)
        {
            switch (route)
            {
                ...
                case "uitoolkit/audit-uss-styles":
                    return "Audit USS authoring and ownership contracts. Hard errors include grouped selector lists, margin or padding shorthand, empty selector blocks, resetting a loaded shared font to none or initial, fixed font size combined with effective auto sizing, fully inlineable single-consumer selectors, invariant declarations retained only by state anchors, repeated declaration bundles, redundant component classes, and page-scoped styles that cross reusable component ownership. It also reports ineffective or redundant layout, text, and background scale-mode declarations while preserving bounded flex, visual, interaction, state, and measured optical contracts.";
                case "uitoolkit/audit-uxml-layout":
                    return "Audit authored UXML for required UI Builder image previews, cross-panel UXML imports, including unused and indirect templates, Label background images that belong on dedicated VisualElements, empty labels without a text binding or authored design-time preview even when they declare a runtime text owner, bound-property literal fallbacks, common production placeholder literals, tooltip attributes, unconsumed element names, layout-only balance, counterweight, spacer, or shim elements, fully fixed flex partitions, non-overlapping absolute sibling stacks that manually reconstruct Flex flow, fixed localized button widths that should use min-width plus padding, fixed content-only Label heights that should follow text, fixed sizes that override natural Flex content, single-axis ScrollViews fixed on their content-sized cross axis, ineffective flex shrink, removable centering wrappers, visually inert text growth or stretching, repeated inline variants, and declarations already owned by loaded USS or Unity engine initial styles.";
