...
namespace VMUnityAutomation.Editor
{
    ...
    /// supports filtering, and optionally persists to disk (Library/VmAutomationActionHistory.json).
    /// </summary>
    [InitializeOnLoad]
    public static class VmAutomationActionHistory
    {
        // ═══════════════════════════════════════════════════════
        //  State
        // ═══════════════════════════════════════════════════════

        private static readonly List<VmAutomationActionRecord> _history = new List<VmAutomationActionRecord>();
        private static readonly object _lock = new object();
        private static long _nextId;

        private const string PersistencePath = "Library/VmAutomationActionHistory.json";

        /// <summary>Fires on the main thread whenever a new action is recorded.</summary>
        public static event Action<VmAutomationActionRecord> OnActionRecorded;
        ...
        /// <summary>
        /// Record a new action after an automation invocation completes.
        /// Thread-safe — can be called from any thread.
        /// </summary>
        public static void RecordAction(VmAutomationActionRecord record)
        {
            int maxEntries = VmAutomationSettings.ActionHistoryMaxEntries;
        ...
        /// </summary>
        public static List<VmAutomationActionRecord> GetAll()
        {
            lock (_lock)
            {
                return new List<VmAutomationActionRecord>(_history);
            }
        }
        ...
        /// </summary>
        public static List<VmAutomationActionRecord> GetFiltered(string agentFilter, string categoryFilter, string searchText)
        {
            lock (_lock)
            {
                var result = new List<VmAutomationActionRecord>();
        ...
        public static List<VmAutomationActionRecord> GetRecent(int count)
        {
            lock (_lock)
            {
        ...
        internal static VmAutomationActionRecord FindById(long actionId)
        {
            lock (_lock)
                return _history.FirstOrDefault(record => record.Id == actionId);
        }

        internal static VmAutomationActionRecord FindByRequestId(long requestId)
        {
            lock (_lock)
                return _history.LastOrDefault(record => record.RequestId == requestId);
        }

        internal static void NotifyRecordUpdated()
        {
            if (VmAutomationSettings.ActionHistoryPersistence)
                SaveToDisk();
        }
        ...
        public static void Clear()
        {
            ...
            try { VmAutomationPersistenceFile.DeleteIfExists(PersistencePath); }
            catch (Exception ex) { Debug.LogWarning($"[Automation History] Failed to delete persistence file: {ex.Message}"); }
        }
        ...
        private static void SaveToDisk()
        {
            try
            {
                List<VmAutomationActionRecord> snapshot;
                lock (_lock) { snapshot = new List<VmAutomationActionRecord>(_history); }
                ...
                foreach (var r in snapshot)
                {
                    wrapper.records.Add(new HistoryEntry
                    {
                        ...
                        undoGroupName = r.UndoGroupName ?? "",
                        undoStatus = r.UndoStatus ?? "unavailable",
                        undoUnavailableReason = r.UndoUnavailableReason ?? "",
                    });
                }

                string json = JsonUtility.ToJson(wrapper, true);
                VmAutomationPersistenceFile.WriteAllText(PersistencePath, json);
            }
            catch (Exception ex)
            {
                Debug.LogWarning($"[Automation History] Failed to save: {ex.Message}");
            }
        }
        ...
        private static void LoadFromDisk()
        {
            try
            {
                if (!VmAutomationPersistenceFile.TryReadAllText(PersistencePath, out string json)) return;
                ...
                lock (_lock)
                {
                    ...
                    foreach (var entry in wrapper.records)
                    {
                        ...
                        _history.Add(new VmAutomationActionRecord
                        {
                            Id              = entry.id,
                            Timestamp       = ts,
                            ...
                            UndoGroupName   = entry.undoGroupName,
                            UndoStatus      = entry.undoStatus == "available" ||
                                              entry.undoStatus == "undone"
                                ? "unavailable"
                                : entry.undoStatus,
                            UndoUnavailableReason = entry.undoStatus == "available" ||
                                                    entry.undoStatus == "undone"
                                ? "domain_reloaded"
                                : entry.undoUnavailableReason,
                        });
                    }
                ...
                Debug.Log($"[Automation History] Loaded {wrapper.records.Count} records from disk.");
            }
            catch (Exception ex)
            {
                Debug.LogWarning($"[Automation History] Failed to load: {ex.Message}");
            }
        }
