...
namespace VMUnityAutomation.Editor
{
    ...
    /// </summary>
    public static class VmAutomationTerrainCommands
    {
        // ─────────────────────────────────────────────
        //  CREATE / DELETE
        // ─────────────────────────────────────────────
        ...
        public static object CreateTerrain(Dictionary<string, object> args)
        {
            ...
            return new Dictionary<string, object>
            {
                { "success", true },
                { "name", name },
                { "instanceId", VmObjectId.Get(terrainGO) },
                { "dataPath", dataPath },
        ...
        public static object GetTerrainInfo(Dictionary<string, object> args)
        {
            ...
            {
                { "name", terrain.name },
                { "instanceId", VmObjectId.Get(terrain.gameObject) },
            ...
            VmAutomationTransformSerialization.AddWorld(result, terrain.transform);
            return result;
        }
        ...
        public static object ListTerrains(Dictionary<string, object> args)
        {
            ...
            foreach (var t in terrains)
            {
                ...
                {
                    { "name", t.name },
                    { "instanceId", VmObjectId.Get(t.gameObject) },
                    { "size", Vec3Dict(t.terrainData.size) },
                    { "isActive", t == Terrain.activeTerrain },
                };
                VmAutomationTransformSerialization.AddWorld(terrainInfo, t.transform);
