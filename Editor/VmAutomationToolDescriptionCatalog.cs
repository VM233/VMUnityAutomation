        ...
        internal static string Get(string route)
        {
            switch (route)
            {
                ...
                case "uitoolkit/audit-uss-styles":
                    return "Audit USS selectors that serve exactly one authored UXML element, hard-error repeated declaration bundles across independently assignable simple classes, redundant authored classes that merely alias a component's inherent class below a named scope, page-scoped style families that cross a reusable component root to skin runtime-generated direct children, declarations that repeat the concrete component's effective baseline, flex-shrink declarations with no finite parent main-axis extent, layout-only flex parents that repeat the cross size of runtime-generated direct children, and non-default declarations whose overly broad target is reset to the Unity engine initial value by an ancestor-scoped branch, while preserving real placement, bounded flex layout, skin-variant, pseudo-state, and runtime-state contracts.";
