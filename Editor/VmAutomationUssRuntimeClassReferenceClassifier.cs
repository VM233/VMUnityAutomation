...
{
    internal static class VmAutomationUssRuntimeClassReferenceClassifier
    {
        ...
        internal static void Index(string path, string text, UssUsageIndex index)
        {
            ...
            foreach (Match match in classApiCallRegex.Matches(text))
            {
                ...
                var line = VmAutomationUssStyleSheetParser.GetLineNumber(text, match.Index);
                var column = VmAutomationUssStyleSheetParser.GetColumnNumber(text, match.Index);
