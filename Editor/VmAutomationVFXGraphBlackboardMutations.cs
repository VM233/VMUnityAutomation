...
{
    internal static class VmAutomationVFXGraphBlackboardMutations
    {
            ...
            VmAutomationVFXGraphMutationContext context, Dictionary<string, object> operation)
        {
            ...
            Set(category, "collapsed", VmAutomationVFXGraphMutationContext.GetBool(operation,
                "collapsed", false));
            int index = operation.ContainsKey("index")
                ? VmAutomationVFXGraphMutationContext.GetInt(operation, "index")
                : categories.Count;
            ...
            VmAutomationVFXGraphMutationContext context, Dictionary<string, object> operation)
        {
            ...
            string previousName = VmAutomationVFXReflection.Get(category, "name")?.ToString() ?? "";
            string name = VmAutomationVFXGraphMutationContext.GetString(operation, "name",
                previousName);
            ...
            if (!string.Equals(name, previousName, StringComparison.Ordinal))
            {
                foreach (UnityEngine.Object parameter in context.Session.Models.Where(
                             model => VmAutomationVFXReflection.HasBaseType(model.GetType(),
                                 VmAutomationVFXReflection.ParameterTypeName) && string.Equals(
                                 VmAutomationVFXReflection.Get(model, "category")?.ToString(),
                                 previousName, StringComparison.Ordinal)))
                    Set(parameter, "category", name);
            }
            Set(category, "name", name);
            if (operation.TryGetValue("collapsed", out object collapsed))
                Set(category, "collapsed", VmAutomationVFXValueCodec.ConvertTo(collapsed,
            ...
            VmAutomationVFXGraphMutationContext context, Dictionary<string, object> operation)
        {
            IList categories = Categories(context);
            int index = ResolveCategory(categories, operation);
            string name = VmAutomationVFXReflection.Get(categories[index], "name")?.ToString() ?? "";
            ...
            List<UnityEngine.Object> parameters = context.Session.Models.Where(model =>
                ...
                string.Equals(VmAutomationVFXReflection.Get(model, "category")?.ToString(),
            ...
            VmAutomationVFXGraphMutationContext context, Dictionary<string, object> operation)
        {
            ...
            int newIndex = VmAutomationVFXGraphMutationContext.GetInt(operation, "index", -1);
            ...
            VmAutomationVFXGraphMutationContext context, Dictionary<string, object> operation)
        {
            ...
            string description = VmAutomationVFXGraphMutationContext.GetString(operation,
                "description");
            ...
            if (operation.TryGetValue("expanded", out object expanded))
                SetCustomAttributeExpanded(context, name,
                    (bool)VmAutomationVFXValueCodec.ConvertTo(expanded, typeof(bool),
                        "operation.expanded"));
            ...
            VmAutomationVFXGraphMutationContext context, Dictionary<string, object> operation)
        {
            ...
            string newName = VmAutomationVFXGraphMutationContext.GetString(operation, "name",
                oldName);
            bool wasExpanded = VmAutomationVFXReflection.Get(descriptor,
                "isExpanded") is bool currentExpanded && currentExpanded;
            if (!string.Equals(oldName, newName, StringComparison.Ordinal))
            {
                bool renamed = Convert.ToBoolean(VmAutomationVFXReflection.Invoke(
                    context.Session.Graph, "TryRenameCustomAttribute", oldName,
                    newName));
                if (!renamed)
                    throw VmAutomationVFXError.Create("custom_attribute_conflict",
            ...
            {
                string current = VmAutomationVFXReflection.Get(descriptor, "type")?.ToString() ?? "";
                string requested = typeValue?.ToString() ?? "";
                if (!string.Equals(current, requested, StringComparison.OrdinalIgnoreCase))
                {
                    bool used = Convert.ToBoolean(VmAutomationVFXReflection.Invoke(
                        context.Session.Graph, "IsCustomAttributeUsed", newName));
                    if (used && !VmAutomationVFXGraphMutationContext.GetBool(operation,
                            "removeUsages", false))
                        throw VmAutomationVFXError.Create("custom_attribute_in_use",
                            $"Custom attribute '{newName}' is in use; set removeUsages=true to remove those models before changing its type.");
                    string desc = VmAutomationVFXReflection.Get(descriptor,
                        "description")?.ToString() ?? "";
                    VmAutomationVFXReflection.Invoke(context.Session.Graph,
            ...
            VmAutomationVFXGraphMutationContext context, Dictionary<string, object> operation)
        {
            ...
            VmAutomationVFXReflection.Invoke(context.Session.Graph, "RemoveCustomAttribute",
                name);
            Dirty(context);
            ...
            VmAutomationVFXGraphMutationContext context, Dictionary<string, object> operation)
        {
            ...
            return Result("groupIndex", index, "title",
                VmAutomationVFXReflection.Get(group, "title")?.ToString() ?? "");
        }
            ...
            VmAutomationVFXGraphMutationContext context, Dictionary<string, object> operation)
        {
            ...
            return Result("stickyNoteIndex", index, "title",
                VmAutomationVFXReflection.Get(note, "title")?.ToString() ?? "");
        }
            ...
            VmAutomationVFXGraphMutationContext context, Dictionary<string, object> operation)
        {
            ...
            Rect bounds = (Rect)VmAutomationVFXValueCodec.ConvertTo(rawBounds, typeof(Rect),
                "operation.bounds");
            Set(UI(context), "uiBounds", bounds);
            Dirty(context);
            return Result("bounds", VmAutomationVFXValueCodec.Sanitize(bounds), "updated", true);
        }

        private static void ApplyUIInfo(VmAutomationVFXGraphMutationContext context,
            ...
            bool allowContents)
        {
            ...
            if (requireFields || operation.ContainsKey("position"))
            {
                ...
                Set(info, "position", VmAutomationVFXValueCodec.ConvertTo(raw, typeof(Rect),
                    "operation.position"));
            }
            ...
            Dictionary<string, object> operation, bool requireFields)
        {
            ...
            if (requireFields || operation.ContainsKey("position"))
            {
                object raw = operation.TryGetValue("position", out object position)
                    ? position : throw new ArgumentException("position is required.");
                Set(note, "position", VmAutomationVFXValueCodec.ConvertTo(raw, typeof(Rect),
                    "operation.position"));
            }
            ...
            if (operation.TryGetValue("colorTheme", out object colorTheme))
                Set(note, "colorTheme", VmAutomationVFXValueCodec.ConvertTo(colorTheme,
                    typeof(int), "operation.colorTheme"));
        }

        private static Array BuildGroupContents(VmAutomationVFXGraphMutationContext context,
            object rawContents)
        {
            List<object> selectors = VmAutomationVFXGraphMutationContext.AsList(rawContents) ??
                throw new ArgumentException("contents must be an array.");
            Type nodeIdType = VmAutomationVFXReflection.RequireType(
                VmAutomationVFXReflection.NodeIdTypeName);
        ...
        private static void AddCustomAttributeCore(
            VmAutomationVFXGraphMutationContext context, string name, string typeName,
            string description)
        {
            Type valueType = VmAutomationVFXReflection.FindType(
                VmAutomationVFXReflection.ValueTypeName) ??
                throw new MissingMemberException(VmAutomationVFXReflection.ValueTypeName);
            object enumValue = VmAutomationVFXValueCodec.ConvertTo(typeName, valueType,
                "operation.valueType");
            ...
            bool success = Convert.ToBoolean(VmAutomationVFXReflection.InvokeMethod(method,
        ...
        private static object FindCustomAttribute(
            VmAutomationVFXGraphMutationContext context, string name)
        {
            object descriptor = VmAutomationVFXReflection.Enumerate(VmAutomationVFXReflection.Get(
                    context.Session.Graph, "customAttributes"))
                .FirstOrDefault(item => string.Equals(
                    VmAutomationVFXReflection.Get(item, "attributeName")?.ToString(), name,
                    StringComparison.OrdinalIgnoreCase));
            if (descriptor == null)
                throw VmAutomationVFXError.Create("model_not_found",
                    $"Custom VFX attribute '{name}' was not found.");
            if (VmAutomationVFXReflection.Get(descriptor, "isReadOnly") is bool readOnly &&
                readOnly)
        ...
        private static void SetCustomAttributeOrder(
            VmAutomationVFXGraphMutationContext context, string name, int index)
        {
            int count = VmAutomationVFXReflection.Enumerate(VmAutomationVFXReflection.Get(
        ...
        private static void SetCustomAttributeExpanded(
            VmAutomationVFXGraphMutationContext context, string name, bool expanded)
        {
            VmAutomationVFXReflection.Invoke(context.Session.Graph,
                "SetCustomAttributeExpanded", name, expanded);
        }

        private static object UI(VmAutomationVFXGraphMutationContext context)
        {
            return VmAutomationVFXReflection.Get(context.Session.Graph, "UIInfos") ??
                   throw new MissingMemberException(
                       context.Session.Graph.GetType().FullName, "UIInfos");
        }

        private static IList Categories(VmAutomationVFXGraphMutationContext context)
        {
            object ui = UI(context);
            IList categories = VmAutomationVFXReflection.Get(ui, "categories") as IList;
            if (categories != null)
                return categories;
            ...
            Dictionary<string, object> operation)
        {
            ...
            {
                int index = (int)VmAutomationVFXValueCodec.ConvertTo(rawIndex,
            ...
            List<int> matches = Enumerable.Range(0, categories.Count).Where(index =>
                string.Equals(VmAutomationVFXReflection.Get(categories[index], "name")
            ...
            int exceptIndex)
        {
            if (Enumerable.Range(0, categories.Count).Any(index =>
                    index != exceptIndex && string.Equals(VmAutomationVFXReflection.Get(
        ...
        {
            Type arrayType = VmAutomationVFXReflection.GetMemberType(target, member) ??
                throw new MissingMemberException(target.GetType().FullName, member);
            return VmAutomationVFXReflection.Get(target, member) as Array ??
        ...
        private static void RemoveStickyReferences(object ui, int removedIndex)
        {
            ...
            {
                Array contents = VmAutomationVFXReflection.Get(group, "contents") as Array;
                if (contents == null)
                    continue;
                List<object> kept = contents.Cast<object>().Where(item =>
                    !(VmAutomationVFXReflection.Get(item, "isStickyNote") is bool sticky &&
                      sticky && Convert.ToInt32(VmAutomationVFXReflection.Get(item, "id")) ==
                      removedIndex)).ToList();
        ...
        private static void ReindexStickyReferences(object ui, int pivot, int delta)
        {
            foreach (object group in GetArray(ui, "groupInfos"))
            {
                ...
                for (int index = 0; index < contents.Length; index++)
                {
                    ...
                    if (!(VmAutomationVFXReflection.Get(item, "isStickyNote") is bool sticky) ||
                        !sticky)
                        continue;
                    int id = Convert.ToInt32(VmAutomationVFXReflection.Get(item, "id"));
        ...
        private static void Dirty(VmAutomationVFXGraphMutationContext context)
        {
            object ui = UI(context);
            ...
            string key, int count)
        {
            if (!operation.ContainsKey(key))
                throw new ArgumentException(key + " is required.");
            int index = VmAutomationVFXGraphMutationContext.GetInt(operation, key, -1);
