...
{
    internal static class MCPVFXReflection
    {
        ...
        internal static Type FindType(string fullName)
        {
            ...
            Type resolved = MCPAssetGraphUtility.FindType(fullName);
            ...
            ICollection<SlotReference> result)
        {
            ...
            if (depth > MCPVFXLimits.SlotDepth)
                throw MCPVFXError.Create("response_too_large",
                    $"VFX slot selector '{selector}' exceeds the supported recursive depth of {MCPVFXLimits.SlotDepth}.");
            if (result.Count >= MCPVFXLimits.SlotsPerModelDirection)
                throw MCPVFXError.Create("response_too_large",
                    $"A VFX model exposes more than {MCPVFXLimits.SlotsPerModelDirection} recursive slots in one direction.");
