        ...
        internal static string Get(string route)
        {
            switch (route)
            {
                ...
                case "uitoolkit/audit-uss-styles":
                    return "Audit USS selectors that serve exactly one authored UXML element, hard-error fully inlineable single-consumer simple selectors even when an allow-single-use marker is present, hard-error instance-owned display and margin declarations left in a one-consumer class only because another selector retains that class as an anchor, hard-error repeated declaration bundles across independently assignable simple classes, redundant authored classes that merely alias a component's inherent class below a named scope, page-scoped style families that cross a reusable component root to skin runtime-generated direct children, declarations that repeat the concrete component's effective baseline, flex-shrink declarations with no finite parent main-axis extent, layout-only flex parents that repeat a cross size already established by authored or runtime-generated in-flow children, fixed-size absolute overlays whose left/top exactly recalculate a fixed-size centered parent's placement, and non-default declarations whose overly broad target is reset to the Unity engine initial value by an ancestor-scoped branch, while preserving real placement, bounded flex layout, visual and interaction regions, edge anchors, measured optical offsets, skin-variant, pseudo-state, and runtime-state contracts.";
