...
{
    internal static class MCPVFXGraphValidateCommands
    {
        internal static object Validate(Dictionary<string, object> args)
        {
            ...
            try
            {
                ...
                if (mode == "reimport")
                {
                    ...
                    if (!MCPVFXGraphSession.TryOpen(assetPath, out session,
                            out object reopenedError))
                        return reopenedError;
                }
                else if (mode == "compile")
                {
                    compileOutput = MCPVFXReflection.Invoke(session.Graph,
                        "RecompileIfNeeded", false, false);
                    if (session.AssetKind == "graph")
                        MCPVFXReflection.Invoke(session.Graph,
                            "CompileAndUpdateAsset", session.Asset);
                    MCPVFXReflection.Invoke(session.Resource, "WriteAsset");
                    AssetDatabase.ImportAsset(assetPath,
                        ImportAssetOptions.ForceUpdate |
                        ImportAssetOptions.ForceSynchronousImport);
                    if (!MCPVFXGraphSession.TryOpen(assetPath, out session,
                ...
                List<Dictionary<string, object>> allDiagnostics =
                    MCPVFXGraphInfoCommands.BuildDiagnostics(session.Graph,
            ...
            int sourceOffset, int maxSourceChars, int shaderCount)
        {
            ...
            {
                string name = MCPVFXReflection.Invoke(resource,
                    "GetShaderSourceName", index)?.ToString() ?? "";
                string source = includeSource
                    ? MCPVFXReflection.Invoke(resource,
                ...
                if (includeSource)
                {
                    ...
                    if (sourceCharacters >
                        MCPVFXLimits.ShaderSourceCharsPerRequest)
                        throw MCPVFXError.Create("response_too_large",
                            $"Generated VFX shader sources exceed {MCPVFXLimits.ShaderSourceCharsPerRequest} characters in one request. Reduce maxShaders or request sources individually.");
                }
        ...
        {
            return Convert.ToInt32(MCPVFXReflection.Invoke(resource,
            ...
            string methodName)
        {
            ...
            return Bounded(MCPVFXReflection.Enumerate(list), methodName)
            ...
            UnityEngine.Object asset)
        {
            ...
            return Bounded(MCPVFXReflection.Enumerate(list),
                ...
                new Dictionary<string, object>
                {
                    ...
                    { "type", (MCPVFXReflection.Get(property, "type") as Type)?.FullName ??
                              MCPVFXReflection.Get(property, "type")?.ToString() ?? "" },
                }).ToList();
        }
        ...
        private static Dictionary<string, object> CompileOutputSummary(object output)
        {
            ...
            return new Dictionary<string, object>
            {
                ...
                { "expressionCount", Count(MCPVFXReflection.Get(output,
                    "expressions")) },
                { "systemCount", Count(MCPVFXReflection.Get(output,
                    "systemDesc")) },
                { "taskCount", Count(MCPVFXReflection.Get(output,
                    "taskDesc")) },
                { "shaderSourceCount", Count(MCPVFXReflection.Get(output,
                    "shaderSourceDesc")) },
                { "sourceDependencies", Bounded(MCPVFXReflection.Enumerate(
                    MCPVFXReflection.Get(output, "sourceDependencies")),
        ...
        {
            List<T> result = values.Take(MCPVFXLimits.CollectionItems + 1)
                .ToList();
            if (result.Count > MCPVFXLimits.CollectionItems)
                throw MCPVFXError.Create("response_too_large",
                    $"VFX validation exposes more than {MCPVFXLimits.CollectionItems} {label}.");
            return result;
        }

        private static object InstancingReason(MCPVFXGraphSession session,
            object compileOutput)
        {
            object reason = MCPVFXReflection.Get(compileOutput,
        ...
        private static int Count(object value)
        {
            ...
            return MCPVFXReflection.Enumerate(value).Count();
        }
            ...
            int defaultValue, int min, int max, out int value, out object error)
        {
            try
            {
                value = args != null && args.TryGetValue(key, out object raw) &&
                        raw != null
                    ? (int)MCPVFXValueCodec.ConvertTo(raw, typeof(int), key)
                    : defaultValue;
            }
            catch (Exception exception)
            {
