...
{
    internal static class VmAutomationCinemachineCommands
    {
        public static object Info(Dictionary<string, object> args)
        {
            if (!VmAutomationCapabilityRegistry.IsCapabilityAvailable("cinemachine"))
            ...
            try
            {
                ...
                List<Component> cinemachine = components.Where(IsCinemachineComponent)
                    ...
                    .ThenBy(component => VmAutomationGameObjectCommands.GetHierarchyPath(
        ...
        public static object Transaction(Dictionary<string, object> args)
        {
            ...
            try
            {
                ...
                Undo.SetCurrentGroupName("VM Unity Automation Edit Cinemachine");
            ...
            Dictionary<string, object> operation, GameObject prefabRoot)
        {
            ...
            if (action != "set-property" && action != "set-object-reference" &&
                action != "set-enabled")
            {
                return PreparedOperation.WithError(VmAutomationResponse.Error(
            ...
            {
                return PreparedOperation.WithError(VmAutomationResponse.Error(
                    $"operations[{index}] Cinemachine component '{GetString(operation, "componentType")}' was not found on '{VmAutomationGameObjectCommands.GetHierarchyPath(gameObject)}'.",
                    "component_not_found"));
            }
            ...
            object beforeValue =
                VmAutomationComponentCommands.GetSerializedValue(property, 2, 32);
            ...
            GameObject prefabRoot)
        {
            ...
            if (prefabRoot == null)
            {
                if (selector.TryGetValue("instanceId", out object instanceId))
                    return VmObjectId.ToObject(instanceId) as GameObject;
            ...
            List<GameObject> matches = prefabRoot.GetComponentsInChildren<Transform>(true)
                ...
                .Where(gameObject =>
                    ...
                    string.Equals(VmAutomationGameObjectCommands.GetHierarchyPath(gameObject), path,
                        StringComparison.Ordinal) ||
                    VmAutomationGameObjectCommands.GetHierarchyPath(gameObject)
                ...
                case "component":
                    ...
                    if (component == null)
                        throw new ArgumentException(
                            $"Target component '{componentType}' was not found on '{VmAutomationGameObjectCommands.GetHierarchyPath(target)}'.");
                    return component;
                default:
                    throw new ArgumentException(
            ...
            bool includeProperties, int maxProperties)
        {
            ...
            {
                { "gameObjectPath", VmAutomationGameObjectCommands.GetHierarchyPath(component.gameObject) },
            ...
            if (iterator.NextVisible(true))
            {
                do
                {
                    ...
                    properties.Add(new Dictionary<string, object>
                    {
                        ...
                        { "value", VmAutomationAssetGraphUtility.SanitizeValue(
                            VmAutomationComponentCommands.GetSerializedValue(iterator, 2, 32)) },
                    });
        ...
        private sealed class PreparedOperation
        {
            ...
            internal Dictionary<string, object> Describe()
            {
                return new Dictionary<string, object>
                {
                    { "index", Index },
                    { "action", Action },
                    { "gameObjectPath", VmAutomationGameObjectCommands.GetHierarchyPath(
                        Component.gameObject) },
                    { "componentType", Component.GetType().FullName },
                    { "propertyPath", PropertyPath },
                    ...
                    { "requested", Reference != null
                        ? VmAutomationGameObjectCommands.GetHierarchyPath(
            ...
            {
                Undo.RecordObject(Component, "VM Unity Automation Edit Cinemachine");
                if (Action == "set-enabled")
                {
                ...
                EditorUtility.SetDirty(Component);
                var result = Describe();
                result["after"] = VmAutomationComponentCommands.GetSerializedValue(
