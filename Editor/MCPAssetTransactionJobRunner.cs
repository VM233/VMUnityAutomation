...
namespace VMUnityAutomation.Editor
{
    ...
    /// </summary>
    internal static class MCPAssetTransactionJobRunner
    {
        ...
        internal static object DryRun(Dictionary<string, object> args)
        {
            try
            {
                MCPAssetTransactionPlan plan = MCPAssetTransactionPlan.Build(args);
            ...
            catch (MCPAssetTransactionPlan.ValidationException exception)
            {
                return MCPResponse.Error(exception.Message, exception.ErrorCode);
            }
        }
            ...
            out object error)
        {
            try
            {
                MCPAssetTransactionPlan.Build(args);
                error = null;
                return true;
            }
            catch (MCPAssetTransactionPlan.ValidationException exception)
            {
                error = MCPResponse.Error(exception.Message, exception.ErrorCode);
                return false;
            }
        }

        internal static bool ExecutePhase(MCPWorkspaceJob job)
        {
            if (job == null || job.JobType != JobType) return false;
            switch (job.Phase)
            {
                case MCPWorkspaceJobRunner.WaitingForEditorPhase:
                    Prepare(job);
                    return true;
                case PreparedPhase:
                    job.Phase = ApplyingPhase;
                    job.StatusMessage = "Prepared snapshots are durable; applying operation 0.";
                    MCPWorkspaceJobRunner.Persist(job);
                    return true;
                case ApplyingPhase:
                    ApplyNext(job);
                    return true;
                ...
                case PublishingPhase:
                    ...
                    return true;
                case MCPWorkspaceJobRunner.VerifyingPhase:
                    VerifyAndCommit(job);
                    return true;
        ...
        internal static void FailAndRollback(MCPWorkspaceJob job, Exception exception)
        {
            if (job == null) return;
            Dictionary<string, object> originalError = exception is
                MCPAssetTransactionPlan.ValidationException validation
                ? MCPResponse.Error(validation.Message, validation.ErrorCode, false)
                : MCPResponse.Error(exception.GetBaseException().Message,
                    "asset_transaction_failed", false);
            Rollback(job, originalError);
        }

        internal static void FailAndRollback(MCPWorkspaceJob job,
            Dictionary<string, object> originalError)
        {
            if (job == null) return;
            Rollback(job, originalError ?? MCPResponse.Error(
                "Asset transaction compilation failed.", "compilation_failed", false));
        }

        internal static void RecoverAfterReload(MCPWorkspaceJob job)
        {
            if (job == null || job.JobType != JobType || job.IsTerminal) return;
            Dictionary<string, object> state = job.TransactionState;
            if (state == null)
            {
                FailWithoutMutation(job, MCPResponse.Error(
            ...
            if (job.Phase == PreparedPhase)
            {
                try
                {
                    MCPAssetTransactionSnapshotStore.VerifySnapshotArtifacts(job.JobId, state);
                    job.StatusMessage =
                ...
                {
                    CompleteRollbackFailed(job, MCPResponse.Error(
            ...
            {
                Rollback(job, MCPResponse.Error(
            ...
            if (job.Phase == MCPWorkspaceJobRunner.AwaitingCompilationStartPhase ||
                job.Phase == MCPWorkspaceJobRunner.CompilingPhase)
            {
                ...
                job.Phase = MCPWorkspaceJobRunner.RequestingCompilationPhase;
                job.StatusMessage =
            ...
            if (job.Phase == MCPWorkspaceJobRunner.WaitingForDomainReloadPhase &&
                job.CompilationFinished)
            {
                if (job.CompilationSucceeded != true)
                {
                    Rollback(job,
                        MCPWorkspaceJobRunner.BuildUnrecordedCompilationOutcomeError(job));
                    return;
                }
                job.AssemblyReloadObserved = true;
                job.Phase = MCPWorkspaceJobRunner.VerifyingPhase;
                job.StatusMessage =
        ...
        internal static bool HasCrossedMutationBoundary(MCPWorkspaceJob job)
        {
            if (job?.JobType != JobType) return false;
            if (job.Phase == ApplyingPhase || job.Phase == SavingPhase ||
                job.Phase == PublishingPhase || job.Phase == RollingBackPhase ||
                job.Phase == MCPWorkspaceJobRunner.RequestingCompilationPhase ||
                job.Phase == MCPWorkspaceJobRunner.AwaitingCompilationStartPhase ||
                job.Phase == MCPWorkspaceJobRunner.CompilingPhase ||
                job.Phase == MCPWorkspaceJobRunner.WaitingForDomainReloadPhase ||
                job.Phase == MCPWorkspaceJobRunner.VerifyingPhase)
                return true;
            return GetInt(job.TransactionState, "nextOperationIndex") > 0;
        }

        internal static void CancelBeforeMutation(MCPWorkspaceJob job)
        {
            if (job?.JobType != JobType) return;
            MCPAssetTransactionSnapshotStore.Cleanup(job.JobId);
            if (job.TransactionState != null)
                job.TransactionState["recoveryArtifactsRetained"] = false;
        }

        internal static object CleanupRecoveryArtifacts(MCPWorkspaceJob job)
        {
            if (job?.JobType != JobType)
                return MCPResponse.Error("The job is not an asset transaction.", "job_not_cleanable");
            if (!job.IsTerminal)
                return MCPResponse.Error("The transaction is not terminal.", "job_not_terminal");
            if (!HasRecoveryArtifacts(job))
                return MCPResponse.Error("The transaction has no retained recovery artifacts.",
                    "job_not_cleanable");
        ...
        internal static bool HasRecoveryArtifacts(MCPWorkspaceJob job)
        {
            return job?.JobType == JobType && job.TransactionState != null &&
                   GetBool(job.TransactionState, "recoveryArtifactsRetained") &&
                   MCPAssetTransactionSnapshotStore.SnapshotDirectoryExists(job.JobId);
        }

        private static void Prepare(MCPWorkspaceJob job)
        {
            MCPAssetTransactionPlan plan = MCPAssetTransactionPlan.Build(job.Request);
            Dictionary<string, object> state = plan.ToPersistentState();
            MCPAssetTransactionSnapshotStore.CaptureBaseline(job.JobId, plan, state);
        ...
        private static void ApplyNext(MCPWorkspaceJob job)
        {
            ...
            state["checkpointEvidence"] =
                MCPAssetTransactionSnapshotStore.CaptureCurrentEvidence(state);
            job.StatusMessage = $"Applied and checkpointed operation {index + 1} of {operations.Count}.";
            MCPWorkspaceJobRunner.Persist(job);
        }

        private static void SaveAndPublish(MCPWorkspaceJob job)
        {
            Dictionary<string, object> state = RequireState(job);
            ...
            MCPAssetTransactionPlan.VerifyPostconditions(state);
            MoveToCompilationOrVerification(job);
        }

        private static void ResumePublishing(MCPWorkspaceJob job)
        {
            Dictionary<string, object> state = RequireState(job);
            if (!VerifyCommitEvidence(state, out List<string> differences))
            {
        ...
        private static void ReconcilePublishedCandidate(MCPWorkspaceJob job)
        {
            Dictionary<string, object> state = RequireState(job);
            if (VerifyCommitEvidence(state, out _))
            {
                ...
                {
                    Rollback(job, MCPResponse.Error(exception.GetBaseException().Message,
                        "transaction_postcondition_failed", false));
                }
                return;
            }

            if (MCPAssetTransactionSnapshotStore.VerifyEvidence(
                    state.TryGetValue("baselineEvidence", out object baseline) ? baseline : null,
                    out _))
            {
                CompleteRolledBack(job, MCPResponse.Error(
        ...
        private static void MoveToCompilationOrVerification(MCPWorkspaceJob job)
        {
        ...
        private static void VerifyAndCommit(MCPWorkspaceJob job)
        {
            Dictionary<string, object> state = RequireState(job);
            if (!VerifyCommitEvidence(state, out List<string> differences))
            {
        ...
        private static void Rollback(MCPWorkspaceJob job,
            Dictionary<string, object> originalError)
        {
            Dictionary<string, object> state = job.TransactionState;
            if (state == null ||
            ...
            try
            {
                ...
                errors.AddRange(MCPAssetTransactionSnapshotStore.RestoreAndVerify(
                    job.JobId, state));
        ...
        private static void CompleteRolledBack(MCPWorkspaceJob job,
            Dictionary<string, object> originalError)
        {
            Dictionary<string, object> state = RequireState(job);
            job.Result = BuildTerminalResult(job, RolledBackPhase);
            job.Error = new Dictionary<string, object>(originalError ?? MCPResponse.Error(
        ...
        private static void CompleteRollbackFailed(MCPWorkspaceJob job,
        ...
        private static void CompleteOutcomeUncertain(MCPWorkspaceJob job,
            string message, List<string> differences)
        {
            ...
            job.Error = MCPResponse.Error(message, "outcome_uncertain", false,
                new Dictionary<string, object>
                {
        ...
        private static void FailWithoutMutation(MCPWorkspaceJob job,
            Dictionary<string, object> originalError)
        {
            ...
            job.CompletedAt = DateTime.UtcNow;
            if (MCPAssetTransactionSnapshotStore.SnapshotDirectoryExists(job.JobId))
                MCPAssetTransactionSnapshotStore.Cleanup(job.JobId);
            MCPWorkspaceJobRunner.Persist(job);
        }

        private static Dictionary<string, object> BuildTerminalResult(MCPWorkspaceJob job,
        ...
        {
            return MCPAssetTransactionSnapshotStore.VerifyEvidence(
        ...
        private static object ExecuteOperation(Dictionary<string, object> operation)
        {
            ...
            switch (type)
            {
                ...
                case "copy":
                {
                    ...
                    EnsureFolderPath(MCPAssetTransactionPlan.NormalizeAssetPath(
                ...
                case "serialized-set":
                {
                    object result = MCPSerializedObjectCommands.SetForTransaction(operation);
                    if (MCPResponse.TryGetError(result, out string message,
        ...
        private static void ResetCompilationEvidence(MCPWorkspaceJob job)
        {
        ...
        private static Dictionary<string, object> GetOriginalError(MCPWorkspaceJob job)
        {
            return job?.TransactionState != null &&
                ...
                : MCPResponse.Error("Asset transaction rollback resumed after reload.",
                    "asset_transaction_interrupted_during_rollback", false);
        }

        private static Dictionary<string, object> RequireState(MCPWorkspaceJob job)
        {
