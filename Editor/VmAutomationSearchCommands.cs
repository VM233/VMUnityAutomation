...
namespace VMUnityAutomation.Editor
{
    ...
    /// </summary>
    public static class VmAutomationSearchCommands
    {
        // ─── Find By Component ───
        ...
        public static object SearchScene(Dictionary<string, object> args)
        {
            ...
            if (!string.IsNullOrWhiteSpace(name) && useRegex)
            {
                ...
                {
                    return VmAutomationResponse.Error(exception.Message, "invalid_name_regex");
                }
            }
            ...
            if (!string.IsNullOrWhiteSpace(layerValue))
            {
                ...
                if (layer < 0 || layer > 31)
                    return VmAutomationResponse.Error("layer must be a valid layer name or an index from 0 to 31.", "invalid_layer");
            }

            var matches = new List<GameObject>();
            var allObjects = VmAutomationObjectSearch.Find<GameObject>(includeInactive);
            ...
            var ordered = matches
                ...
                .ThenBy(item => VmObjectId.Get(item.GameObject), StringComparer.Ordinal)
                .ToList();
            var results = ordered
                .Skip(offset)
                .Take(limit)
                .Select(item => new Dictionary<string, object>
                {
                    ...
                    { "path", item.Path },
                    { "instanceId", VmObjectId.Get(item.GameObject) },
        ...
        public static object FindByComponent(Dictionary<string, object> args)
        {
            ...
            var objects = VmAutomationObjectSearch.Find(componentType, includeInactive);
            ...
            foreach (var obj in objects)
            {
                ...
                if (results.Count < limit)
                {
                    results.Add(new Dictionary<string, object>
                    {
                        ...
                        { "instanceId", VmObjectId.Get(comp.gameObject) },
        ...
        public static object FindByShader(Dictionary<string, object> args)
        {
            ...
            var results = new List<Dictionary<string, object>>();
            var renderers = VmAutomationObjectSearch.Find<Renderer>(includeInactive: true);
                        ...
                        mat.shader.name.IndexOf(shaderName, StringComparison.OrdinalIgnoreCase) >= 0)
                    {
                        ...
                        if (results.Count < limit)
                        {
                            results.Add(new Dictionary<string, object>
                            {
                                ...
                                { "instanceId", VmObjectId.Get(renderer.gameObject) },
                                { "material", mat.name },
