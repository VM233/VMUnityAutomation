...
{
    internal static class MCPVFXAssetSettings
    {
        ...
        private sealed class Descriptor
        {
            ...
            internal Type ResolveValueType()
            {
                return string.IsNullOrEmpty(EnumTypeName)
                    ? ValueType
                    : MCPVFXReflection.FindType(EnumTypeName);
            }
        }
        ...
        private static readonly Descriptor[] Descriptors =
        {
            ...
            new Descriptor("instancingMode", "m_Infos.m_InstancingMode", null,
                MCPVFXReflection.InstancingModeTypeName),
            ...
            MCPVFXGraphSession session)
        {
            ...
            foreach (Descriptor descriptor in Descriptors)
            {
                if (TryRead(session, descriptor, out object value))
                    result[descriptor.Name] = MCPVFXValueCodec.Sanitize(value);
            }
            return result;
        }
            ...
            MCPVFXGraphSession session)
        {
            ...
            object value = available
                ? MCPVFXReflection.Invoke(session.Graph, "GetCompilationMode")
                : null;
        ...
        internal static void Restore(MCPVFXGraphSession session,
        ...
        internal static Dictionary<string, object> Set(
            MCPVFXGraphSession session, string name, object rawValue)
        {
            if (session == null || session.AssetKind != "graph")
        ...
        internal static bool TrySetCompilationMode(MCPVFXGraphSession session,
        ...
        private static Dictionary<string, object> Summary(
            MCPVFXGraphSession session, Descriptor descriptor)
        {
        ...
        private static bool TryRead(MCPVFXGraphSession session,
        ...
        private static object Write(MCPVFXGraphSession session,
            Descriptor descriptor, object rawValue, bool validate)
        {
            if (!(session?.Resource is UnityEngine.Object resource) ||
                resource == null)
                throw MCPVFXError.Create("vfx_resource_unavailable",
            ...
            object value = MCPVFXValueCodec.ConvertTo(rawValue, valueType,
                "operation.value");
            if (validate)
        ...
        private static void SetCompilationMode(MCPVFXGraphSession session,
            object rawValue)
        {
            Type valueType = MCPVFXReflection.FindType(
                MCPVFXReflection.CompilationModeTypeName);
            ...
            MCPVFXReflection.Invoke(session.Graph, "SetCompilationMode", value,
                false);
        }
