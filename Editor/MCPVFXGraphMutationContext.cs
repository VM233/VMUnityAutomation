...
{
    internal sealed class MCPVFXGraphMutationContext
    {
        ...
        internal MCPVFXGraphMutationContext(MCPVFXGraphSession session)
        {
            Session = session ?? throw new ArgumentNullException(nameof(session));
        }

        internal MCPVFXGraphSession Session { get; }
            ...
            string argumentName = "nodeId")
        {
            ...
            if (model == null)
                throw MCPVFXError.Create(MissingModelCode(selector),
            ...
            string argumentName = "parameterId")
        {
            ...
            if (model == null)
                throw MCPVFXError.Create(MissingModelCode(definitionId),
            ...
            string parentSelector, int index, Dictionary<string, object> operation)
        {
            ...
            UnityEngine.Object model = MCPVFXReflection.Invoke(descriptor,
            ...
            Dictionary<string, object> operation)
        {
            ...
            UnityEngine.Object parameter = MCPVFXReflection.Invoke(descriptor,
                "CreateInstance") as UnityEngine.Object;
            if (parameter == null)
                throw new InvalidOperationException(
            ...
            try
            {
                MCPVFXReflection.Invoke(Session.Graph, "AddChild", parameter, -1);
            ...
            Dictionary<string, object> values, bool requireName)
        {
            ...
            {
                Type valueType = MCPVFXReflection.Get(parameter, "type") as Type ??
        ...
        {
            int nodeId = Convert.ToInt32(MCPVFXReflection.Invoke(parameter,
                "AddNode", position));
            object node = MCPVFXReflection.Invoke(parameter, "GetNode", nodeId);
        ...
        internal void RemoveParameterNode(string selector)
        {
            ...
            MCPVFXReflection.Invoke(parameter, "RemoveNode", node);
        }
        ...
        internal void RemoveModel(UnityEngine.Object model)
        {
            if (MCPVFXReflection.HasBaseType(model.GetType(),
                    MCPVFXReflection.ParameterTypeName))
            {
                foreach (object node in MCPVFXReflection.Enumerate(
                             MCPVFXReflection.Get(model, "nodes")).ToList())
                    MCPVFXReflection.Invoke(model, "RemoveNode", node);
            }
            Type modelType = MCPVFXReflection.RequireType(
                MCPVFXReflection.ModelTypeName);
            MCPVFXReflection.Invoke(modelType, "RemoveModel", model);
        }

        internal void MoveBlock(UnityEngine.Object block,
            UnityEngine.Object context, int index)
        {
            if (!MCPVFXReflection.HasBaseType(block.GetType(),
                    MCPVFXReflection.BlockTypeName))
                throw new ArgumentException("nodeId does not identify a VFX block.");
            if (!MCPVFXReflection.HasBaseType(context.GetType(),
                    MCPVFXReflection.ContextTypeName))
                throw new ArgumentException(
                    "parentContextId does not identify a VFX context.");
            int childCount = MCPVFXReflection.Enumerate(
                MCPVFXReflection.Get(context, "children")).Count();
            if (index < 0 || index > childCount)
                throw new ArgumentOutOfRangeException(nameof(index),
                    $"index must be between 0 and {childCount}.");
            MCPVFXReflection.Invoke(context, "AddChild", block, index);
        }

        internal void ApplyCommonModelFields(UnityEngine.Object model,
            Dictionary<string, object> values, string path)
        {
            bool isContext = MCPVFXReflection.HasBaseType(model.GetType(),
                MCPVFXReflection.ContextTypeName);
            bool isBlock = MCPVFXReflection.HasBaseType(model.GetType(),
                MCPVFXReflection.BlockTypeName);
            ...
            object rawValue, string path)
        {
            ...
            object setting = MCPVFXReflection.Enumerate(settings).FirstOrDefault(item =>
                string.Equals(MCPVFXReflection.Get(item, "name")?.ToString(), name,
                    StringComparison.Ordinal));
            if (setting == null)
                throw MCPVFXError.Create("setting_not_found",
                    $"{path} is not a setting on {model.GetType().FullName}.");
            FieldInfo field = MCPVFXReflection.Get(setting, "field") as FieldInfo;
            Type valueType = field?.FieldType ??
                             MCPVFXReflection.Get(setting, "value")?.GetType();
            object converted = ConvertValue(rawValue, valueType, path);
            MCPVFXReflection.Invoke(model, "SetSettingValue", name, converted);
        }
            ...
            string path)
        {
            ...
            IReadOnlyList<MCPVFXReflection.SlotReference> all =
                MCPVFXReflection.EnumerateSlots(model, member);
            List<MCPVFXReflection.SlotReference> selectorMatches = all
            ...
            if (path.StartsWith("[", StringComparison.Ordinal))
                throw MCPVFXError.Create("slot_not_found",
                    $"{direction} slot selector '{path}' was not found on model '{MCPVFXReflection.SemanticName(model)}'.");

            List<MCPVFXReflection.SlotReference> matches = all.Where(item =>
            {
                object slot = item.Slot;
                string slotPath = MCPVFXReflection.Get(slot, "path")?.ToString();
                string slotName = MCPVFXReflection.Get(
                    MCPVFXReflection.Get(slot, "property"), "name")?.ToString();
                return string.Equals(slotPath, path, StringComparison.Ordinal) ||
                       string.Equals(slotName, path, StringComparison.Ordinal);
            }).ToList();
            if (matches.Count == 0)
                throw MCPVFXError.Create("slot_not_found",
                    $"{direction} slot '{path}' was not found on model '{MCPVFXReflection.SemanticName(model)}'.");
            if (matches.Count > 1)
                throw MCPVFXError.Create("slot_not_found",
            ...
            int? toParameterNodeId)
        {
            ...
            bool canLink = Convert.ToBoolean(MCPVFXReflection.Invoke(output,
                "CanLink", input));
            bool reverseCanLink = Convert.ToBoolean(MCPVFXReflection.Invoke(input,
                "CanLink", output));
            if (!canLink || !reverseCanLink)
                throw MCPVFXError.Create("data_link_incompatible",
                    $"Unity rejected data link {fromPath} -> {toPath}.");
            bool linked = Convert.ToBoolean(MCPVFXReflection.Invoke(output,
                "Link", input));
            if (!linked)
                throw MCPVFXError.Create("data_link_incompatible",
            ...
            UnityEngine.Object toModel, string toPath)
        {
            object output = ResolveSlot(fromModel, "output", fromPath);
            object input = ResolveSlot(toModel, "input", toPath);
            if (!MCPVFXReflection.Enumerate(MCPVFXReflection.Get(output,
                    "LinkedSlots")).Contains(input))
                throw MCPVFXError.Create("data_link_incompatible",
                    "The requested VFX data link does not exist.");
            MCPVFXReflection.Invoke(output, "Unlink", input);
        ...
        {
            Type contextType = MCPVFXReflection.RequireType(
                MCPVFXReflection.ContextTypeName);
            bool canLink = Convert.ToBoolean(MCPVFXReflection.Invoke(contextType,
                "CanLink", from, to, fromIndex, toIndex));
            if (!canLink)
                throw MCPVFXError.Create("flow_link_incompatible",
                    $"Unity rejected flow link {fromIndex} -> {toIndex}.");
            MCPVFXReflection.Invoke(from, "LinkTo", to, fromIndex, toIndex);
        }
            ...
            UnityEngine.Object to, int toIndex)
        {
            List<object> outputSlots = MCPVFXReflection.Enumerate(
                MCPVFXReflection.Get(from, "outputFlowSlot")).ToList();
            ...
            bool exists = MCPVFXReflection.Enumerate(MCPVFXReflection.Get(
                    outputSlots[fromIndex], "link")).Any(link =>
                    ReferenceEquals(MCPVFXReflection.Get(link, "context"), to) &&
                    Convert.ToInt32(MCPVFXReflection.Get(link, "slotIndex") ??
                        -1) == toIndex);
            if (!exists)
                throw MCPVFXError.Create("flow_link_incompatible",
                    $"The requested VFX flow link {fromIndex} -> {toIndex} does not exist.");
            MCPVFXReflection.Invoke(from, "UnlinkTo", to, fromIndex, toIndex);
        }

        internal Dictionary<string, object> AliasIds()
        {
            Dictionary<string, object> result = aliases.ToDictionary(pair => pair.Key,
                pair => (object)MCPVFXReflection.StableId(pair.Value),
                StringComparer.Ordinal);
            foreach (KeyValuePair<string, KeyValuePair<UnityEngine.Object, int>> pair
                     in occurrenceAliases)
                result[pair.Key] = MCPVFXReflection.StableId(pair.Value.Key) + ":" +
                                   pair.Value.Value;
            return result;
        }
        ...
        {
            return MCPResponse.ToDictionary(value);
        }
            ...
            string name)
        {
            bool duplicate = Session.Models.Any(model => model != current &&
                MCPVFXReflection.HasBaseType(model.GetType(),
                    MCPVFXReflection.ParameterTypeName) &&
                string.Equals(MCPVFXReflection.Get(model, "exposedName")?.ToString(),
                    name, StringComparison.Ordinal));
            if (duplicate)
                throw MCPVFXError.Create("parameter_name_conflict",
            ...
            int? parameterNodeId, object output, object input)
        {
            ...
            object targetNode = RequireParameterNode(model, parameterNodeId.Value,
                MCPVFXReflection.StableId(model) + ":" + parameterNodeId.Value);
            RemoveParameterOccurrenceLink(model, output, input);
            object list = MCPVFXReflection.Get(targetNode, "linkedSlots");
            if (list == null)
            {
            ...
            object output, object input)
        {
            if (!MCPVFXReflection.HasBaseType(model.GetType(),
                ...
                return;
            foreach (object node in MCPVFXReflection.Enumerate(
                         MCPVFXReflection.Get(model, "nodes")))
            {
                object list = MCPVFXReflection.Get(node, "linkedSlots");
                if (!(list is IList mutable))
                    continue;
                for (int index = mutable.Count - 1; index >= 0; index--)
                {
                    object pair = mutable[index];
                    if (ReferenceEquals(MCPVFXReflection.Get(pair, "outputSlot"),
                            output) &&
                        ReferenceEquals(MCPVFXReflection.Get(pair, "inputSlot"),
                            input))
        ...
        private static void SetTypedMember(object target, string memberName,
            object rawValue, string path)
        {
            Type type = MCPVFXReflection.GetMemberType(target, memberName);
            if (type == null)
        ...
        {
            if (!MCPVFXReflection.TrySet(target, memberName, value))
                throw new MissingMemberException(
        ...
        {
            return MCPResponse.TryGetError(response, out string _,
        ...
        private object ResolveDescriptor(string kind, string catalogId)
        {
            ...
            if (descriptorCache.TryGetValue(key, out object descriptor))
                return descriptor;
            if (!MCPVFXGraphCatalogCommands.TryResolveModelDescriptor(kind,
                    catalogId, true, out descriptor, out object resolveError))
                throw MCPVFXError.Create(ErrorCode(resolveError,
            ...
            string path)
        {
            try
            {
                return MCPVFXValueCodec.ConvertTo(rawValue, targetType, path);
            }
