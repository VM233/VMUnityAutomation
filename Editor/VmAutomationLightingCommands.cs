...
namespace VMUnityAutomation.Editor
{
    ...
    /// </summary>
    public static class VmAutomationLightingCommands
    {
        public static object GetLightingInfo(Dictionary<string, object> args)
        {
            var lights = VmAutomationObjectSearch.Find<Light>();
            ...
            foreach (var light in lights)
            {
                var info = new Dictionary<string, object>
                {
                    ...
                    { "instanceId", VmObjectId.Get(light.gameObject) },
        ...
        public static object CreateLight(Dictionary<string, object> args)
        {
            ...
            if (args.ContainsKey("position"))
                go.transform.position = VmAutomationGameObjectCommands.DictToVector3(args["position"] as Dictionary<string, object>);

            if (args.ContainsKey("rotation"))
                go.transform.eulerAngles = VmAutomationGameObjectCommands.DictToVector3(args["rotation"] as Dictionary<string, object>);
        ...
        public static object CreateReflectionProbe(Dictionary<string, object> args)
        {
            ...
            if (args.ContainsKey("size"))
                probe.size = VmAutomationGameObjectCommands.DictToVector3(args["size"] as Dictionary<string, object>);
            ...
            var result = new Dictionary<string, object>
            {
                ...
                { "name", go.name },
                { "instanceId", VmObjectId.Get(go) },
                { "size", VmAutomationGameObjectCommands.Vector3ToDict(probe.size) },
            };
            VmAutomationTransformSerialization.AddWorld(result, go.transform);
            return result;
        }
