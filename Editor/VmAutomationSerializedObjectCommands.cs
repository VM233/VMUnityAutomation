...
{
    public static class VmAutomationSerializedObjectCommands
    {
        ...
        {
            return Set(args, saveOnlyTarget: false);
        }

        internal static object SetForTransaction(Dictionary<string, object> args)
        {
            return Set(args, saveOnlyTarget: true);
        }
        ...
        private static object Set(Dictionary<string, object> args, bool saveOnlyTarget)
        {
            ...
            int maxDepth = Math.Max(1, Math.Min(GetInt(args, "maxDepth", 3), 8));
            int maxArrayElements = Math.Max(1, Math.Min(GetInt(args, "maxArrayElements", 50), 500));
            var beforeValue = VmAutomationComponentCommands.GetSerializedValue(property, maxDepth, maxArrayElements);
            try
            {
                VmAutomationComponentCommands.SetSerializedValue(property, args["value"]);
            }
            catch (Exception exception)
            {
                serialized.Update();
                return VmAutomationResponse.Error(exception.Message, "serialized_object_set_failed", false,
            ...
            out string error)
        {
            ...
            {
                target = VmObjectId.ToObject(instanceId);
                if (target != null)
                    return true;
        ...
        private static Type ResolveType(string typeName)
        {
            ...
            return VmAutomationComponentCommands.FindType(typeName) ??
            ...
            int maxArrayElements)
        {
            return new Dictionary<string, object>
            {
                ...
                { "value", VmAutomationComponentCommands.GetSerializedValue(property, maxDepth, maxArrayElements) },
            };
        }
