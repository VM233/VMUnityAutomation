...
{
    internal static class VmAutomationTransformSerialization
    {
            ...
            Vector3 defaultValue)
        {
            if (Approximately(value, defaultValue) == false)
                target[key] = VmAutomationGameObjectCommands.Vector3ToDict(value);
        }
            ...
            Quaternion value, Vector3 eulerAngles)
        {
            if (Mathf.Abs(Quaternion.Dot(value, Quaternion.identity)) < 1f - 0.000001f)
                target[key] = VmAutomationGameObjectCommands.Vector3ToDict(eulerAngles);
        }
