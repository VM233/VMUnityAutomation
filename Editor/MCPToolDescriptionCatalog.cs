        ...
        internal static string Get(string route)
        {
            switch (route)
            {
                ...
                case "instance/resolve":
                    return "Resolve one Unity Editor MCP instance by exact project path or port.";
                ...
                case "uitoolkit/audit-uss-styles":
                    return "Audit USS selectors that serve exactly one authored UXML element, hard-error repeated declaration bundles across independently assignable simple classes, redundant authored classes that merely alias a component's inherent class below a named scope, page-scoped style families that cross a reusable component root to skin runtime-generated direct children, declarations that repeat the concrete component's effective baseline, layout-only flex parents that repeat the cross size of runtime-generated direct children, and non-default declarations whose overly broad target is reset to the Unity engine initial value by an ancestor-scoped branch, while preserving real placement, skin-variant, pseudo-state, and runtime-state contracts.";
                case "uitoolkit/audit-uxml-layout":
                    return "Audit authored UXML for tooltip attributes, unconsumed element names, fully fixed flex partitions, layout-only sibling groups manually reconstructed with absolute offsets, fixed cross sizes that override natural in-flow Flex content, ineffective flex-shrink on default vertical ScrollView content, fixed cross-axis content wrappers inside single-axis ScrollViews, layout-only manually centered containers, removable single-child centering wrappers, visually inert centered-label stretching or growth, repeated inline layout variants, and inline declarations already owned by loaded USS or Unity engine initial styles.";
                ...
                case "vfxgraph/component-control":
                    return "Control one exact VisualEffect component in Play Mode: play, stop, pause, resume, reinitialize, step or simulate, send typed events, and set or reset runtime exposed-property overrides.";
                ...
                case "undo/history":
                    return "List persisted MCP action identities and their directional Undo availability; this is not Unity's global Undo stack.";
                case "undo/clear":
