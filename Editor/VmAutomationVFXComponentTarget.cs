...
{
    internal sealed class VmAutomationVFXComponentTarget : IDisposable
    {
        private readonly GameObject prefabRoot;
        private bool disposed;

        private VmAutomationVFXComponentTarget(Component component, string scope,
        ...
        internal static bool TryResolve(Dictionary<string, object> selector,
            bool allowPrefab, out VmAutomationVFXComponentTarget target, out object error)
        {
            target = null;
            Type visualEffectType = VmAutomationVFXReflection.FindType(
                     ...
                     })
            {
                if (GetString(selector, key).Length >
                    VmAutomationVFXLimits.SelectorCharacters)
                {
                    error = VmAutomationResponse.Error(
                        $"{key} cannot exceed {VmAutomationVFXLimits.SelectorCharacters} characters.",
                        "invalid_arguments");
                    return false;
                }
            }
            if (!string.IsNullOrEmpty(prefabPath) &&
                !VmAutomationVFXAssetPath.TryNormalizeFile(prefabPath, false,
                    out prefabPath, out string prefabPathError))
            {
                error = VmAutomationResponse.Error(prefabPathError,
                    "invalid_arguments");
                return false;
            }
            ...
            try
            {
                ...
                target = new VmAutomationVFXComponentTarget(exactComponent ??
            ...
            catch (Exception exception)
            {
                ...
                error = VmAutomationResponse.Error(failure.Message,
            ...
            Type targetType, string valuePath)
        {
            ...
            bool clear = reference.TryGetValue("clear", out object clearValue) &&
                         (bool)VmAutomationVFXValueCodec.ConvertTo(clearValue,
                    ...
                    out object componentInstanceId))
            {
                ...
                if (exact == null || !targetType.IsInstanceOfType(exact))
                    throw VmAutomationVFXError.Create("component_not_found",
            ...
            else
            {
                ...
                if (string.IsNullOrWhiteSpace(indexPath) || indexPath.Length >
                    VmAutomationVFXLimits.SelectorCharacters)
                    throw new ArgumentException(
                        $"{valuePath}.hierarchyIndexPath is required and cannot exceed {VmAutomationVFXLimits.SelectorCharacters} characters.");
            ...
            if (gameObject == null)
                throw VmAutomationVFXError.Create("game_object_not_found",
            ...
            {
                componentIndex = (int)VmAutomationVFXValueCodec.ConvertTo(rawIndex,
                    typeof(int), valuePath + ".componentIndex");
            }
            else
            {
                if (components.Length > 1)
                    throw VmAutomationVFXError.Create("component_selector_ambiguous",
        ...
        private void RequireSameScope(GameObject gameObject, string valuePath)
        {
            ...
            if (!sameScope)
                throw VmAutomationVFXError.Create("component_selector_mismatch",
        ...
        internal static string HierarchyPath(GameObject gameObject)
        {
            ...
            while (current != null)
            {
                if (depth++ >= VmAutomationVFXLimits.HierarchyDepth)
                    throw VmAutomationVFXError.Create("response_too_large",
                        $"GameObject hierarchy exceeds depth {VmAutomationVFXLimits.HierarchyDepth}.");
                names.Push(current.name);
                current = current.parent;
            }
            string result = string.Join("/", names);
            if (result.Length > VmAutomationVFXLimits.SelectorCharacters)
                throw VmAutomationVFXError.Create("response_too_large",
                    $"GameObject hierarchy path exceeds {VmAutomationVFXLimits.SelectorCharacters} characters.");
            return result;
        }
        ...
        internal static string HierarchyIndexPath(GameObject gameObject)
        {
            ...
            if (result.Length > VmAutomationVFXLimits.SelectorCharacters)
                throw VmAutomationVFXError.Create("response_too_large",
                    $"GameObject hierarchy index path exceeds {VmAutomationVFXLimits.SelectorCharacters} characters.");
            return result;
        }
            ...
            string scenePath)
        {
            ...
            {
                componentCount++;
                if (componentCount > VmAutomationVFXLimits.CollectionItems)
                    throw VmAutomationVFXError.Create("response_too_large",
                        $"Loaded scenes expose more than {VmAutomationVFXLimits.CollectionItems} VisualEffect components.");
            ...
            GameObject root, Type componentType)
        {
            ...
            if (components.Count > VmAutomationVFXLimits.CollectionItems)
                throw VmAutomationVFXError.Create("response_too_large",
                    $"Prefab exposes more than {VmAutomationVFXLimits.CollectionItems} VisualEffect components.");
            return components;
        }
            ...
            string path, ref int inspected)
        {
            if (root == null)
                return new List<GameObject>();
            inspected++;
            if (inspected > VmAutomationVFXLimits.HierarchyObjects)
                throw VmAutomationVFXError.Create("response_too_large",
                    $"GameObject selector inspected more than {VmAutomationVFXLimits.HierarchyObjects} hierarchy objects.");
        ...
        {
            if (roots.Count > VmAutomationVFXLimits.HierarchyObjects)
                throw VmAutomationVFXError.Create("response_too_large",
                    $"Scene exposes more than {VmAutomationVFXLimits.HierarchyObjects} root GameObjects.");
            string[] parts = path.Trim('/').Split('/');
            if (parts.Length > VmAutomationVFXLimits.HierarchyDepth)
                throw VmAutomationVFXError.Create("response_too_large",
                    $"hierarchyIndexPath exceeds depth {VmAutomationVFXLimits.HierarchyDepth}.");
            ...
            ICollection<GameObject> result)
        {
            ...
            while (pending.Count > 0)
            {
                ...
                if (result.Count >= VmAutomationVFXLimits.HierarchyObjects)
                    throw VmAutomationVFXError.Create("response_too_large",
                        $"Component discovery inspected more than {VmAutomationVFXLimits.HierarchyObjects} hierarchy objects.");
