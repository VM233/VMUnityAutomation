...
{
    internal static class MCPVFXSettingsCommands
    {
        ...
        {
            internal MCPVFXGraphSession Session;
        ...
        internal static object Info(Dictionary<string, object> args)
        {
            ...
            if (scope != "" && scope != "project" && scope != "user")
                return MCPResponse.Error("scope must be project, user, or omitted.",
                    "invalid_arguments");
            try
            {
        ...
        internal static object Transaction(Dictionary<string, object> args)
        {
            ...
            try
            {
                for (int index = 0; index < rawOperations.Count; index++)
                {
                    Dictionary<string, object> operation =
                        MCPVFXGraphMutationContext.AsDictionary(rawOperations[index]) ??
            ...
            catch (Exception exception)
            {
                ...
                {
                    return MCPResponse.Error(
                        $"VFX settings transaction failed: {MCPVFXReflection.Unwrap(exception).Message}. Rollback failed: {MCPVFXReflection.Unwrap(rollbackException).Message}",
                        "vfx_transaction_rollback_failed");
                }
                Exception failure = MCPVFXReflection.Unwrap(exception);
                return MCPResponse.Error(
                    failure.Message, MCPVFXError.Code(failure,
            ...
            string name, object rawValue, int index)
        {
            ...
            switch (property.propertyType)
            {
                case SerializedPropertyType.Float:
                    property.doubleValue = (float)MCPVFXValueCodec.ConvertTo(
                        rawValue, typeof(float),
                        $"operations[{index}].value");
                    break;
                case SerializedPropertyType.Integer:
                    property.longValue = (int)MCPVFXValueCodec.ConvertTo(
                        rawValue, typeof(int),
                        $"operations[{index}].value");
                    break;
                case SerializedPropertyType.ObjectReference:
                    ...
                    property.objectReferenceValue = (UnityEngine.Object)
                        MCPVFXValueCodec.ConvertTo(rawValue, objectType,
                            $"operations[{index}].value");
                    break;
                default:
            ...
            switch (property.propertyType)
            {
                ...
                case SerializedPropertyType.ObjectReference:
                    ...
                    if (name == "runtimeResources")
                        return MCPVFXReflection.FindType(
        ...
        {
            Type library = MCPVFXReflection.RequireType(
                MCPVFXReflection.LibraryTypeName);
            object binder = MCPVFXReflection.Get(library, "currentSRPBinder");
            return MCPVFXReflection.Get(binder, "SRPAssetTypeStr")?.ToString() ??
        ...
        private static void SetPreference(string name, object rawValue, int index)
        {
            ...
            if (!TryPreferenceDescriptor(name, out string key, out Type type))
                throw MCPVFXError.Create("unsupported_vfx_version",
            ...
            if (name == "cameraBuffersFallback")
                ...
                EditorPrefs.SetBool(key, (bool)MCPVFXValueCodec.ConvertTo(
                    rawValue, typeof(bool), $"operations[{index}].value"));
            else if (type == typeof(int))
            {
                int value = (int)MCPVFXValueCodec.ConvertTo(rawValue,
            ...
            {
                float value = (float)MCPVFXValueCodec.ConvertTo(rawValue,
            ...
            out string key, out Type valueType)
        {
            ...
            if (!PreferenceKeys.TryGetValue(name, out string fieldName))
                return false;
            Type preferenceType = MCPVFXReflection.FindType(
                MCPVFXReflection.ViewPreferenceTypeName);
            if (preferenceType == null || preferenceType.GetMethod("SetDirty",
                    System.Reflection.BindingFlags.Static |
                    System.Reflection.BindingFlags.Public |
                    System.Reflection.BindingFlags.NonPublic) == null)
                return false;
            key = MCPVFXReflection.Get(preferenceType, fieldName)?.ToString();
            ...
            valueType = name == "authoringPrewarmMaxTime" ? typeof(float) :
                ...
                name == "cameraBuffersFallback"
                    ? MCPVFXReflection.FindType(
                        MCPVFXReflection.CameraBufferFallbackTypeName)
                    : typeof(bool);
        ...
        private static Type PreferenceType(string name)
        {
            ...
            if (name == "cameraBuffersFallback")
                return MCPVFXReflection.RequireType(
                    MCPVFXReflection.CameraBufferFallbackTypeName);
            return typeof(bool);
        }
        ...
        private static object ReadSerialized(SerializedProperty property)
        {
            ...
            switch (property.propertyType)
            {
                ...
                case SerializedPropertyType.ObjectReference:
                    return MCPVFXValueCodec.Sanitize(
        ...
        private static void MarkPreferencesDirty()
        {
            Type type = MCPVFXReflection.RequireType(
                MCPVFXReflection.ViewPreferenceTypeName);
            MCPVFXReflection.Invoke(type, "SetDirty");
        }
        ...
        private static List<GraphCompilationSnapshot> CaptureCompilationModes()
        {
            ...
            {
                MCPVFXGraphSession session = OpenGraphForCompilationMode(path);
                snapshots.Add(new GraphCompilationSnapshot
                {
                    Session = session,
                    Mode = MCPVFXReflection.Invoke(session.Graph,
                        "GetCompilationMode"),
                });
            }
            return snapshots;
        ...
        private static void ApplyCompilationMode(
            ...
            bool reimport)
        {
            Type modeType = MCPVFXReflection.RequireType(
                MCPVFXReflection.CompilationModeTypeName);
            object mode = Enum.Parse(modeType, edition ? "Edition" : "Runtime");
            foreach (GraphCompilationSnapshot snapshot in snapshots)
                MCPVFXReflection.Invoke(snapshot.Session.Graph,
        ...
        private static MCPVFXGraphSession OpenGraphForCompilationMode(
            string path)
        {
            if (MCPVFXGraphSession.TryOpen(path, out MCPVFXGraphSession session,
        ...
        private static List<string> VFXAssetPaths()
        {
            ...
            if (guids.Length > MCPVFXLimits.ProjectAssets)
                throw MCPVFXError.Create("response_too_large",
                    $"The project contains more than {MCPVFXLimits.ProjectAssets} VisualEffectAsset records; project-wide VFX mutation is bounded.");
            List<string> paths = guids.Select(AssetDatabase.GUIDToAssetPath)
                .Where(path => !string.IsNullOrEmpty(path))
                .Distinct(StringComparer.Ordinal).ToList();
            if (paths.Count > MCPVFXLimits.ProjectAssets)
                throw MCPVFXError.Create("response_too_large",
                    $"The project contains more than {MCPVFXLimits.ProjectAssets} distinct VisualEffectAsset paths.");
            return paths;
        }
        ...
        private static int ParseCameraFallback(object raw)
        {
            Type type = MCPVFXReflection.RequireType(
                MCPVFXReflection.CameraBufferFallbackTypeName);
            object value = MCPVFXValueCodec.ConvertTo(raw, type,
                "value");
