...
{
    internal sealed class MCPWorkspaceJob
    {
        ...
        internal List<MCPGitPackageExpectation> ExpectedPackages { get; set; } = new();
        ...
        internal static MCPWorkspaceJob FromDictionary(Dictionary<string, object> values)
        {
            if (values == null)
            ...
            job.ExpectedPackages = GetDictionaryList(values, "expectedPackages")
                .Select(MCPGitPackageExpectation.FromDictionary).ToList();
            return job;
        }
