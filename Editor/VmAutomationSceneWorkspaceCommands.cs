...
namespace VMUnityAutomation.Editor
{
    ...
    /// </summary>
    internal static class VmAutomationSceneWorkspaceCommands
    {
        public static object Execute(Dictionary<string, object> args)
        {
            ...
            {
                return VmAutomationResponse.Error(exception.Message, "invalid_arguments");
            }
            switch (action)
            {
                case "list":
        ...
        private static object Open(Dictionary<string, object> args)
        {
            ...
            if (string.IsNullOrEmpty(path))
                return VmAutomationResponse.Error("path is required for action=open.", "invalid_arguments");
            if (!path.EndsWith(".unity", StringComparison.OrdinalIgnoreCase) ||
                AssetDatabase.LoadAssetAtPath<SceneAsset>(path) == null)
                return VmAutomationResponse.Error($"Scene asset '{path}' was not found.", "scene_not_found");
            ...
            switch (modeValue)
            {
                ...
                case "additive":
                    ...
                    break;
                default:
                    return VmAutomationResponse.Error("mode must be single or additive.", "invalid_arguments");
            }
        ...
        private static object Close(Dictionary<string, object> args)
        {
            ...
            if (scene.isDirty)
            {
                ...
                if (save && (string.IsNullOrEmpty(scene.path) || !EditorSceneManager.SaveScene(scene)))
                    return VmAutomationResponse.Error($"Failed to save scene '{scene.name}'.",
                        "scene_save_failed");
            }
            ...
            if (!success)
                return VmAutomationResponse.Error($"Unity failed to close scene '{scene.name}'.",
                    "scene_close_failed");
            var response = BuildWorkspaceResponse();
        ...
        private static object SetActive(Dictionary<string, object> args)
        {
            ...
            if (!SceneManager.SetActiveScene(scene))
                return VmAutomationResponse.Error($"Unity failed to activate scene '{scene.name}'.",
