...
{
    internal sealed class MCPVFXRuntimeState
    {
        ...
        internal Dictionary<string, object> Inspect(Component component,
            ...
            int maxOutputEvents)
        {
            if (MCPVFXReflection.Get(component, "visualEffectAsset") == null)
            ...
            return new Dictionary<string, object>
            {
                { "hasAnySystemAwake", Convert.ToBoolean(
                    MCPVFXReflection.Invoke(component, "HasAnySystemAwake")) },
            ...
            Component component, string name, bool particle, bool spawner)
        {
            ...
            if (particle)
            {
                object info = MCPVFXReflection.Invoke(component,
                    "GetParticleSystemInfo", name);
                result["particleState"] = new Dictionary<string, object>
                {
                    { "aliveCount", MCPVFXValueCodec.Sanitize(
                        MCPVFXReflection.Get(info, "aliveCount")) },
                    { "capacity", MCPVFXValueCodec.Sanitize(
                        MCPVFXReflection.Get(info, "capacity")) },
                    { "sleeping", MCPVFXReflection.Get(info, "sleeping") },
                    { "bounds", MCPVFXValueCodec.Sanitize(
                        MCPVFXReflection.Get(info, "bounds")) },
                };
            }
            if (spawner)
            {
                object state = MCPVFXReflection.Invoke(component,
        ...
        private static Dictionary<string, object> SpawnStateSummary(object state)
        {
            ...
            foreach (string member in new[]
                ...
                result[member] = MCPVFXValueCodec.Sanitize(
                    MCPVFXReflection.Get(state, member));
            return result;
        }
            ...
            string methodName)
        {
            ...
            MCPVFXReflection.Invoke(component, methodName, names);
            if (names.Count > MCPVFXLimits.CollectionItems)
                throw MCPVFXError.Create("response_too_large",
                    $"{methodName} returned more than {MCPVFXLimits.CollectionItems} names.");
        ...
        private void AddDiscovered(int count)
        {
            ...
            if (discoveredRecords > MCPVFXLimits.ReturnedRuntimeRecordsPerRequest)
                throw MCPVFXError.Create("response_too_large",
                    $"Component inspection discovered more than {MCPVFXLimits.ReturnedRuntimeRecordsPerRequest} runtime system and event records.");
        }
    }
}
